...
data class Circle(val center: Point, val radius: Double) {
     ...
     */
    fun contains(p: Point): Boolean {
        return p.distance(center) <= radius == true
    }
}
...
fun lineBySegment(s: Segment): Line =
        Line(s.begin, Math.atan((s.end.y - s.begin.y) / (s.end.x - s.begin.x)) % Math.PI)
...
fun bisectorByPoints(a: Point, b: Point): Line {
    val x = (a.x + b.x) / 2
    val y = (a.y + b.y) / 2
    val ang = Math.PI / 2 + Math.atan((b.y - a.y) / (b.x - a.x))
 ...
 * Описание алгоритмов см. в Интернете1qw
 * (построить окружность по трём точкам, или
 * построить окружность, описанную вокруг треугольника - эквивалентная задача).
 */
fun circleByThreePoints(a: Point, b: Point, c: Point): Circle {
    val lineFirst = bisectorByPoints(a, b)
    val lineSecond = bisectorByPoints(c, b)
    val center = lineFirst.crossPoint(lineSecond)
