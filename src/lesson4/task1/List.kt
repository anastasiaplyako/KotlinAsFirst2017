 ...
 * в начале проверяем кратно ли х заданному числу, далее делаем проверку делителя на простоту,
 * если да, то добавляем в мутуирующий лист, иначе заново цикл
 */
...
fun decimal(digits: List<Int>, base: Int): Int {
    ...
    for (i in digits.size - 1 downTo 0) {
        res += digits[i] * pow(base.toDouble(), degree)
        degree++
    }
    return res.toInt()
}
...
fun russian(n: Int): String {
    ...
    val ed1 = listOf("десять", "одиннадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать", "шестнадцать", "семьнадцать", "восемьнадцать", "девятнадцать")
        ...
        when (i) {
            ...
            3 -> if (x % 100 !in 10..19) {
                when {
                    ...
                    x % 10 == 0 || x % 10 in 5..9 -> res.add(0, ed[x % 10] + "тысяч")
                }
                x /= 10
                res.add(0, doz[x % 10])
            } else {
